...
foreach my $dir (@dirs)
{
    if( open F, "find $dir -type f -name '$pattern' |" )
    {
        while( chomp( my $file = readline F ) )
        {
            ...
            if( -f "$dir/translation/templates/$basename.pot" )
            {
                my $head = read_header($file);

                printf "Updating %-40s", $file;
                system("msgmerge", "-U", "-N", $file, "$dir/translation/templates/$basename.pot");

                write_header($file, $head);
            }
            else
            {
                die "$dir/templates/$basename.pot";
            }
        }
